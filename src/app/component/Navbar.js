...
const Navbar = ({ data }) => {
    ...
    const handleLanguageChange = (language) => {
        ...
        if (
            ...
            logoRedirectUrl = `/${selectedLanguage}`;
        } else {
            logoRedirectUrl = "/";
        }
    };
    return (
        <div className="flex justify-between px-6 lg:px-14 relative">
            <div className="max-w-[180px] w-[30%]"><a href={logoRedirectUrl} className="w-fit"><Image src={logo} className="w-[100%]" alt="logo" /></a></div>
            <ul className="flex items-center gap-10 font-light text-xl ">
                {/* <li>{data.link}</li> */}
                {
                    pathname !== "/contact-us" &&
                    pathname !== "/privacy-policy" &&
                    pathname !== "/blogs" && // Exclude /blogs from the condition
                    (
                        <li>
                            <select
                                className="block w-full text-[1.1rem] text-center text-black border-[#55BF88] bg-[#55bf8847] border-2 p-3 py-2 rounded-[4rem] appearance-none focus:outline-none focus:ring-0"
